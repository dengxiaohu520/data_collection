...
alter TABLE db_user.tb_user_extra add `prepayment_amount` double(10, 2) NULL DEFAULT 0.00 COMMENT '预付费金额（单位元）',
...
CREATE TABLE `tb_fae_order`  (
  ...
  `appeal_state` tinyint(1) NULL DEFAULT 1 COMMENT '申诉状态 1: 无, 2：审核中，3：成立，4：不成立',
...
CREATE TABLE IF NOT EXISTS `tb_fae_user_consult` (
  `id` int(11) NOT NULL AUTO_INCREMENT comment '主键',
  `uid` int(11) NOT NULL AUTO_INCREMENT comment '咨询者id',
  `fae_uid` int(11) NOT NULL AUTO_INCREMENT comment 'fae工程师id',
  `order_no` varchar(128) NOT NULL COMMENT '订单号',
...
CREATE TABLE IF NOT EXISTS `tb_fae_config` (
  ...
  `key` varchar(256) NOT NULL DEFAULT 0.00 COMMENT '关键字',
  `value` varchar(256) NOT NULL DEFAULT 0.00 COMMENT '内容',
